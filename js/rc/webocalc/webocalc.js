...
Added APC 10x10 Â¨EÂ¨ prop to prop list
...
//Put global variables here...these values should remain consistent through the entire code
	V_lipo_cell = 3.6;	//volts per lipo cell under load
	V_A123_cell = 2.8;	//volts per A123 cell under load
	K_lipo = 10.66;		//joules per lipo cell per mAh using 80% discharge. Calculated based on 3.7 V average voltage.
...
//create results cell dynamically, then write to it.
	...
	pagehtml += '<p class="helptext">License: GNU <a href="http://www.gnu.org/copyleft/gpl.html">GPL v. 2</a></p>\n';
...
//	alert("540: run time = " + run_time_minutes);
	...
	lipo_cell_mah_needed = energy_needed/K_lipo;
...
//	alert("398: estimated motor weight: " + motor_weight.toFixed(0) + " grams; Estimated motor Kv: " + guessed_kv.toFixed(0) + " rpm/V.");
	...
	var vbat_min = 0.56 * Math.sqrt(estimated_power);	//obtained empirically Aug 3 2008. Works surprisingly well! (Nov 2009: suggests 1S lipo/ 1S A123 for Slow Stick & similar. Must fix!
...
//Nov 7 2009 - make sure cell counts are > 1S if the model weighs more than some minimum threshold.
	...
	var nlipo_min = find_cell_count(vbat_min,V_lipo_cell);
	var nlipo_max = find_cell_count(vbat_max,V_lipo_cell);
	if((nlipo_min == 1) && (auw > 80.0 )){	//model weighs more than 80 gm, 1 S lipo not a good choice; bump up to 2S
		nlipo_min++;
	}
	while(nlipo_min >= nlipo_max){	//make sure nlipo_max is > nlipo_min after bumping up nlipo_min
		nlipo_max++;
	}
...
//check if the table is already longer than one row - if so, delete previous data.
	...
	for(count = nlipo_min; count <= nlipo_max; count++){
		lipo_pack_mAh = lipo_cell_mah_needed/count;	//how many mAh should the pack have to supply this power for this many minutes?
		lipo_pack_mAh = refine_mAh(lipo_pack_mAh);
		pack_voltage = count * V_lipo_cell;		// What is the voltage of the pack?
		...
		cell_mAh = lipo_pack_mAh.toFixed(0);
...
/*-------------------------------*/
//find number of series lipo cells that is nearest fit to "vbat" volts
//aug 05 2008 
...
//Since A123 cells are only available in one or two capacities, this uses a different algorithm than the one used to find lipo cell count - The pack weight and maximum possible pack output power are used, rather than "C" rate and mAh.
//apr 06 2009 
...
/*-------------------------------*/
function refine_mAh(lipo_pack_mAh){
	var lipo_size = [10,20,30,50,90,130,200,220,250,300,350,400,450,500,600,650,750,800,910,1000,1100,1200,1320,1500,1800,2100,2500,3000,3300,4000,4400,5000,6000,6600,7200,8000,9400,10000,15000,20000];
	var lipo_size_error = [];
	var i,j,imin,imax,jmin,jmax,diff;
	i = 0;
	//skip cell sizes that are obviously too small
	while(lipo_size[i] < lipo_pack_mAh){
		i++
	}
	imin = i;

	//look through remaining typical lipo cell capacities and find error from ideal capacity
	for (i=imin; i < lipo_size.length; i++){
		diff = Math.abs( (lipo_pack_mAh - lipo_size[i])/lipo_pack_mAh);
		lipo_size_error[i] = diff;
	}
	//find pack capacity closest to ideal mAh - look for lowest percentage error
	var min, oldmin, m;
	oldmin = lipo_size_error[imin];
	m = imin;
	for (j=imin; j <= lipo_size.length; j++){
		min = lipo_size_error[j];
		if(min < oldmin){
			oldmin = min;	//update oldmin every time a lower error is found
			m = j;		//save corresponding array index
		}
	}
	var recommended_lipo_size = lipo_size[m];
	return recommended_lipo_size;
}
/*-------------------------------*/
...
//	alert("260: wing loading = " + wing_loading + "gm/dmÂ²");
	var stall_speed;
	var Wldg;
	var rho_air = 1.25; //kg/mÂ³, density of air at sea level
	...
	stall_speed = Math.sqrt(2*Wldg/rho_air);  //metres/sec, calculated from mg = 1/2 Cl rho_air VÂ² S
...
//confirm("1020: Verify rounding of metric flying field length!");
  if(plane.units == "imperial"){
	wl_msg = (plane.wing_loading * 0.3277).toFixed(2) + " oz/square foot.";	//0.3277 converts gm/dmÂ² into oz/sq ft
...
// alert("1324: flight duration = " + desired_duration);
 ...
 celltype = myDoc.getElementById('celltype').value;	/*lipo or a123?*/
...
function find_pack_voltage(myDoc){
	...
	if(celltype == "lipo"){
		packvoltage = V_lipo_cell * cellcount;
	}else if(celltype == "a123"){
